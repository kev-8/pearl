...
for summary in nlp.pipe(test_df):
   proj_tok = [token.lemma_.lower() for token 
               in summary if token.pos_ not in removal
...
lda_model = LdaMulticore(corpus=corpus, id2word=dictionary, iterations=50, 
                         num_topics=10, workers = 2, passes=10)
...
topic_modeling_df['topic'] = [sorted(lda_model[corpus][text])[0][0] for text in
                              range(len(topic_modeling_df['article_text']))]
